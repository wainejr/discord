...
2. **`check_expiration`** — if the YouTube **refresh token** is within 24h of its assumed lifetime (default 7 days from issuance, override with `ACELERADO_REFRESH_TOKEN_TTL_DAYS`), post a renewal reminder to the log channel (rate-limited to once/hour). The 1h access-token TTL is *not* what we count down to: google-auth silently refreshes it on every call. Refresh-token issuance is recorded in a `token_refresh_issued_at.txt` sidecar — written by both the CLI consent flow and `/token renew-finish`, and bootstrapped to `now` on first read if missing (so existing installs upgrading to this code get a conservative 7-day grace before the warning fires; user can re-run `/token renew-start` to anchor the clock honestly). Distinguishes already-expired ("expirou há Xd Yh") from soon-to-expire ("expira em Xh Ym") and points operators at `/token renew-start`.
